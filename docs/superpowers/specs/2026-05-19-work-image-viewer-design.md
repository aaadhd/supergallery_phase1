...
│  [‹]      작품 이미지 전체      [›]  │  ← 양옆 수직 중앙
│                                     │
│ 아바타 작가명  [원래 크기][×N.N][−][+]│  ← 좌하단: 정보 / 우하단: 줌 그룹
│ 작품명                              │
└─────────────────────────────────────┘
```
...
|---|---|---|
| 우상단 | 항상 | ✕ 닫기 |
| 좌측 중앙 | 항상 | ‹ 이전 — scale=1 활성 / scale>1 흐리게 |
| 우측 중앙 | 항상 | › 다음 — scale=1 활성 / scale>1 흐리게 |
| 좌하단 | 항상 | 아바타 + 작가명 + 작품명 |
| 우하단 | 항상 | "원래 크기" · ×N.N 배율 배지 · − · + 한 줄 그룹. scale=1이면 원래크기·배율·−이 흐리게(비활성), scale=MAX이면 + 흐리게 |
