...
> **단일 소스**
> - 화면 문자열은 [Copy_v1.md] 가입·소셜 가입·로그인 영역이 단일 소스다.
> - 수정 시 Copy를 먼저 갱신하고 본 문서를 같은 작업 범위에서 맞춘다.
> - 약관·개인정보처리방침 **전문**은 [Handoff_Terms_v1.md] · [Handoff_Privacy_v1.md] 참조.
...
| 항목 | 결정 | 근거 |
|---|---|---|
| 필수 동의 | 이용약관 + 개인정보 수집·이용 + 만 14세 이상 자기 명시 | [Policy §2.2] |
| 선택 동의 | 마케팅 정보 수신 (단일 항목, 기본 OFF) | [Policy §1]·[§21.1 L-4] |
| 발송 채널 | 사용자 보유 식별자 자동 라우팅 (이메일·알림톡·SMS) | [Policy §1.1] |
| 만 14세 미만 차단 | 시스템 차단 + 약관 자기 명시 + 거짓 입력 책임 + 사후 신고 4단 가드 | [Policy §2.2] |
| 이메일·소셜 동일 구성 | 두 흐름 모두 4종 동의 동일 적용 | [Policy §2.1] |
...
`{provider}` 자리에는 카카오·구글·Apple 표기만 들어간다([Copy_v1.md] 소셜 가입 문구).
...
`{provider}` is replaced with Kakao / Google / Apple brand strings from [Copy_v1.md].
...
모든 UI 문구는 [Copy_v1.md] 가입·소셜 가입·로그인 영역을 따른다. 본 문서는 prose 참고용이며 i18n 키 목록은 적지 않는다.
...
- [ ] 마케팅 동의 보조 설명에 채널 자동 라우팅 명시 ([Policy §1.1] 정합)
...
본 가입 동의 구성은 [Handoff_LegalReview_Checklist_v1.md] §3 LP-1(만 14세 차단)·LP-9(마케팅 동의 로그) 검토 포인트와 직결된다. 변호사 검토 시 본 문서를 약관·처리방침과 함께 동봉.
