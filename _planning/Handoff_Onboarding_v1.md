...
> **단일 소스 안내**
> - 코드(i18n) 단일 소스: [`Copy_v1.md`](Copy_v1.md) 온보딩 카피 키 · 본인 작품 찾기 카피 키 키
...
| 동시 선택 race | type 가드(비회원 슬롯 종류만 승격)로 두 번째 선택 자동 차단 | [CLAUDE.md 초대 자동 연결 정책](../CLAUDE.md) |
...
| 시나리오 | 한국어 | English | 키 |
|---|---|---|---|
| 동시 선택 race (이미 가져감) | 이 자리는 이미 다른 분이 연결됐어요. 본인 작품이 맞다면 작가님께 말씀해 주세요. | This spot has already been claimed. If it's actually your work, please let the artist know. | 본인 작품 찾기 카피 키 |
...
- 진입 안내: 본인 작품 찾기 카피 키 (inactive 토큰 시), 본인 작품 찾기 카피 키, 본인 작품 찾기 카피 키, 본인 작품 찾기 카피 키
...
- [ ] 진입 분기별 배너 자동 분기 (일반·소셜·초대) — 토큰 sessionStorage(단말 보관 키) 보유 여부로 판정
- [ ] Step 2 진입 조건: 활성 또는 inactive 토큰 (Policy v2.16). 0건이면 자동 스킵
- [ ] 동시 선택 race: type 가드(비회원 슬롯 종류만 승격) 후 두 번째 클릭 시 본인 작품 찾기 카피 키 토스트 + 카드 새로고침
- [ ] 명시 클릭 + 확인 다이얼로그 1회 = 자동 연결 방지 (Policy §3.2 자동 연결 옵션 없음)
- [ ] 관심사 태그 15종 ko/en 동일 노출 + 다중 선택 동작
- [ ] 검증 메시지 9종 ko/en 분기
- [ ] 완료 화면 CTA 2종 동작 (경로 /upload, `/`)
- [ ] inactive 토큰 단계 Step 2 진입 시 본인 작품 찾기 카피 키 헤더 한 줄 추가 노출
