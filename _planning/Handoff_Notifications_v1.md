...
- [ ] 인박스 카테고리 필터 7종 동작 (전체·좋아요·팔로우·Pick·응모전·초대·시스템)
...
| 버전 | 일자 | 작성 | 변경 내용 |
|------|------|------|----------|
| v4 | 2026-05-04 | PM × Claude | **기획전 선정 알림 신설 (Policy v2.19·B-3b 연동)** — §4.4 "기획전 선정 알림" 섹션 신설(ko/en, 작품명·기획전 이름 변수 포함). 단위는 작품(piece). 기획전은 영구 배지를 부여하지 않으므로 알림으로만 인지(Policy §15.2 정합). 이후 append — **카테고리 분류 정합**: §1.3 카테고리 표 7행 → 8행(큐레이션 신설). 큐레이션 = Pick + 기획전 선정(운영팀 직권), 응모전 = 응모전 선정 + 공지(응모형), 시스템 = 검수 통과 + 검수 반려 + 초대 수락(시스템 운영) 3분기 명문화. §4·§4.4·§4.5 카테고리 표기 갱신("시스템 강제" → "강제 발송 + 카테고리 표기"). 이후 append — **응모전 표준어 정합 (B-4d-naming)** — 옛 "이벤트(카테고리 의미)"·"응모형 이벤트"를 **응모전**(영문 Contest)으로 통일. §1.3 카테고리 표·§4 알림 종류 본문·매트릭스 표기 정합. 코드 식별자(`categoryEvent`·`evt.*`)는 코드 도메인이라 그대로 유지. |
