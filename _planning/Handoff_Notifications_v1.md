...
> **단일 소스 안내**
> - 코드(i18n) 단일 소스: [`Copy_v1.md`](Copy_v1.md) 알림 카피 영역 · 검수 카피 키 · 신고 카피 키 · 초대 카피 키 · Pick 알림 카피 키 키
...
| 항목 | 한국어 | English | 키 |
|---|---|---|---|
| 제목 | 알림 | Notifications | 알림 카피 키 |
| 일괄 처리 | 모두 읽음 | Mark all read | 알림 카피 키 |
| 일괄 삭제 | 읽은 알림 삭제 | Delete read | 알림 카피 키 |
| 항목 삭제 | 삭제 | Delete | 알림 카피 키 |
| 필터 전체 | 전체 | All | 알림 카피 키 |
| 필터 미독 | 읽지 않음 | Unread | 알림 카피 키 |
| 빈 상태 제목 | 알림이 없어요 | No notifications yet | 알림 카피 키 |
| 빈 상태 미독 | 읽지 않은 알림이 없습니다 | No unread notifications | 알림 카피 키 |
| 빈 상태 안내 | 새로운 소식이 생기면 여기에 표시됩니다 | New updates will show up here. | 알림 카피 키 |
| 설정 링크 | 알림 설정 | Notification settings | 알림 카피 키 |

### 1.2 일괄 처리 확인 다이얼로그

| 시나리오 | 한국어 | English | 키 |
|---|---|---|---|
| 모두 읽음 처리 | 모든 알림을 읽음 처리할까요? | Mark all notifications as read? | 알림 카피 키 |
| 읽은 알림 삭제 | 읽은 알림을 모두 삭제할까요? | Delete all read notifications? | 알림 카피 키 |

### 1.3 카테고리 필터 (6종)

| 카테고리 | 한국어 | English | 키 |
|---|---|---|---|
| 유형 전체 | 유형 전체 | All types | 알림 카피 키 |
| 좋아요 | 좋아요 | Likes | 알림 카피 키 |
| 팔로우 | 팔로우 | Follows | 알림 카피 키 |
| Pick | Pick | Pick | 알림 카피 키 |
| 이벤트 | 이벤트 | Events | 알림 카피 키 |
| 초대 | 초대 | Invite | 알림 카피 키 |
| 시스템 | 시스템 | System | 알림 카피 키 |

### 1.4 시간 표기

| 표기 | 한국어 | English | 키 |
|---|---|---|---|
| 방금 | 방금 | Just now | 알림 카피 키 |
| n분 전 | {n}분 전 | {n}m ago | 알림 카피 키 |
| n시간 전 | {n}시간 전 | {n}h ago | 알림 카피 키 |
| n일 전 | {n}일 전 | {n}d ago | 알림 카피 키 |
...
> - ~~초대 카피 키~~ — 매칭 후보 거부 신호 (yes/no 게이트 폐기)
> - ~~초대 카피 키~~ — 작가의 회원 직접 추가 사후 알림 (자가 해제 진입점 폐기)
> - ~~초대 카피 키~~ — 자가 disavow 후 알림 (자가 해제 진입점 폐기)
...
| 시나리오 | 한국어 | English | 키 |
|---|---|---|---|
| 좋아요 받음 | 님이 회원님의 작품 "{work}"을 좋아합니다 | liked your work "{work}" | 알림 카피 키 |
| 팔로우 받음 | 님이 회원님을 팔로우합니다 | followed you | 알림 카피 키 |
| Pick 선정 | 축하해요! "{work}"이(가) Artier's Pick으로 뽑혔어요 | Congrats! "{work}" was selected as an Artier's Pick | 알림 카피 키 |
| 환영 메시지 | Artier에 오신 것을 환영합니다! 첫 작품을 업로드해 보세요. | Welcome to Artier! Upload your first work. | 알림 카피 키 |
| 이벤트 진행 중 | 「{event}」 이벤트가 진행 중입니다. 지금 참여해 보세요. | Event "{event}" is active — join now. | 알림 카피 키 |
...
|---|---|---|---|---|---|
| 검수 카피 키 | ✓ | — | 자동 라우팅 | 자동 라우팅 | 발행 직후 1건 |
| 검수 카피 키 | ✓ | 자동 라우팅 | 자동 라우팅 | 자동 라우팅 | 검수 승인 시 |
| 검수 카피 키 | ✓ | 자동 라우팅 | 자동 라우팅 | 자동 라우팅 | 클레임된 친구에게(v2.16) |
| 검수 카피 키 | ✓ | 자동 라우팅 | 자동 라우팅 | 자동 라우팅 | 검수 반려 시 |
| 검수 카피 키 | ✓ | 자동 라우팅 | 동의 시 | 동의 시 | 팔로워 알림 |
| 신고 카피 키 | ✓ | 자동 라우팅 | 자동 라우팅 | 자동 라우팅 | 신고로 삭제 시 |
| 신고 카피 키 | ✓ | 자동 라우팅 | 자동 라우팅 | 자동 라우팅 | 신고로 비공개 시 |
| 신고 카피 키 | ✓ | 자동 라우팅 | 자동 라우팅 | 자동 라우팅 | 자동 비공개 발동 시 |
| 신고 카피 키 | ✓ | — | 자동 라우팅 | — | 신고자에게 결과 |
| Pick 카피 키 | ✓ | 자동 라우팅 | 동의 시 | 동의 시 | Pick 선정 시 |
| 초대 카피 키 | ✓ | 자동 라우팅 | 자동 라우팅 | 자동 라우팅 | 친구가 클레임했을 때 |
...
- 라벨: 알림 카피 키, 내부 식별자, 내부 식별자, 삭제 카피 키, 내부 식별자, 내부 식별자, 빈 상태 카피 키, 내부 식별자, 내부 식별자, 설정 링크 카피 키
- 다이얼로그: 알림 카피 키, 내부 식별자
- 카테고리: 알림 카피 키, 내부 식별자, 내부 식별자, 내부 식별자, 내부 식별자, 내부 식별자, 내부 식별자
- 시간: 알림 카피 키, 시간 카피 키, 시간 카피 키, 시간 카피 키
- 시드: 알림 카피 키, 팔로우 시드 알림, Pick 선정 시드 알림, 환영 시드 알림, 이벤트 활성 시드 알림

### 9.2 검수 알림 (review.notif*)
- 검수 카피 키, 내부 식별자, 내부 식별자, 내부 식별자, 내부 식별자

### 9.3 신고 알림 (report.notif*)
- 신고 카피 키, 내부 식별자, 내부 식별자, 내부 식별자
