...
| 이벤트 | 이벤트 | Events | 이벤트 선정 + 이벤트 공지 |
| 시스템 | 시스템 | System | Pick 선정 + 기획전 선정 + 검수 통과 + 검수 반려 + 초대 수락 |
...
→ 운영팀이 매주 Pick 선정 시 작가에게 1건. 시스템 강제(토글 없음). 단위: 전시.
...
→ 운영팀이 기획전 편집에서 piece를 추가할 때 그 piece의 작가에게 1건. 시스템 강제(토글 없음). 단위: 작품(piece). **영구 배지는 부여하지 않음** (Policy §15.2 — 기획전 = 배지 없음). 알림으로만 인지.
...
→ 운영팀이 이벤트 선정작 확정 시 작가에게 1건. 시스템 강제.
...
| 버전 | 일자 | 작성 | 변경 내용 |
|------|------|------|----------|
| v4 | 2026-05-04 | PM × Claude | **기획전 선정 알림 신설 (Policy v2.19·B-3b 연동)** — §4.4 "기획전 선정 알림" 섹션 신설(ko/en, 작품명·기획전 이름 변수 포함). 시스템 카테고리 표에 "기획전 선정" 항목 추가. 단위는 작품(piece). 기획전은 영구 배지를 부여하지 않으므로 알림으로만 인지(Policy §15.2 정합). |
