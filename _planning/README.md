...
| 표준어 | 정의 | 비고 |
|---|---|---|
| **전시(Exhibition)** | 1~10장 이미지로 구성된 갤러리 단위. 업로드 1번 = 전시 1개 | 코드 타입 이름은 전시 (CLAUDE.md 「용어 주의」 참조) |
| **작품** | 전시 안의 개별 이미지 1장 | 코드상 별도 엔티티 없이 전시 안의 `image[i]`로 존재 |
| **그룹명** | 함께 올리기 시 그룹의 이름 | 20자 |
| **작품명·전시명** | 각 콘텐츠 제목 | 각 20자 (TITLE_FIELD_MAX_LEN) |
| **커버** | 전시 카드·썸네일 대표 이미지 | — |
...
| **기획전** | 운영팀이 묶은 주제별 전시 큐레이션 | 개수 제한 없음 (Policy §15.4 v2.5) |
| **이벤트** | 응모·선정 형식의 한시적 행사 | — |
...
| **활성** (active) | 검수 통과 후 친구에게 보낼 수 있는 상태 | — |
| **inactive** | 검수 신청·반려 단계로 일시 비활성. 친구가 클릭해도 본문 미노출(작품 미리보기 그리드만) | Policy v2.16에서 inactive 단계도 친구 클레임 가능 |
| **만료** | 발급일로부터 90일 경과 | — |
| **취소** (revoked) | 작품 삭제·작가 탈퇴 등으로 영구 무효 | — |
