...
다른 `_planning` 문서·소절을 가리키는 인용은 말미 `<!-- 인용 정의 -->`에 **목적지 URL을 한 번만** 적고, 본문에는 **`[라벨](./파일.md#앵커)` 인라인**으로 동일 경로를 둔다. 에디터·미리보기에서 클릭 시 **원문 위치**로 바로 이동한다(순수 참조형 `[라벨]`만 두면 일부 환경에서 말미 정의 줄로 점프한다).
