# Proud Gallery — Planning Documents

**서비스명**: Proud Gallery (SuperGallery Phase 1)
**대상 사용자**: 시니어·중장년 순수미술 작가와 감상자
**성격**: 시니어 친화 UX 중심의 웹 기반 디지털 아트 갤러리 플랫폼
**최종 갱신**: 2026-05-19 (v1.15)

---

## 프로젝트 구성

Proud Gallery Phase 1은 **2트랙 단일 프로젝트**로 구성된다.
...
이 폴더(`_planning/`)는 Proud Gallery 서비스의 **단일 기획 소스(Single Source of Truth)** 다. 외부로 전달하거나 외주 개발사와 공유할 때 이 폴더만 주면 된다.
...
### 화면 스펙 HTML 동기화 의무 (`ProudGallery_Screen_Spec_v1.html`)
...
**코드 주도 변경 시 추가 의무**: HTML 동기화 트리거는 위 표처럼 MD 변경에서 시작되지만, **코드에서만 UI 요소를 제거·변경한 경우** — 예: JSX에서 렌더 제거, `sr-only`로 강등, 플로우 단계 변경 — MD가 갱신되지 않아도 HTML 스펙이 구 UI를 계속 보여주게 된다. 이런 코드 주도 변경이 있으면 Copy·PRD 파일 업데이트와 상관없이 해당 화면 HTML 슬라이드를 **같은 커밋 범위에서** 반드시 수정한다.
...
|---|---|
| **신고·모더레이션** (Policy §12.* — 수치·액션·자동 비공개·알림 매트릭스) | `Policy_v1.md` · `PRD_User_v1.md`(CM-03, §12 플로우, 관련 EXH/PRF) · `PRD_Admin_v1.md`(ADM-RPT 등) · `IA_ScreenList_v1.md`(CM-03, USR-EXH-05 등) · `Copy_v1.md`(신고·시스템 알림 키) · `Handoff_Notifications_v1.md` · `_screen_specs/ProudGallery_Screen_Spec_v1.html`(해당 슬라이드) |
...
| **N종 열거 라벨 변경** (검수 4종·반려 4사유·토큰 4상태·운영자 역할 등) | **SSoT 1곳만 수정**: `Policy_v1.md` 해당 § (§23/§12.1/§3/§17). PRD/IA는 인용 형태라 자동 반영. 사용자 노출 라벨은 `Copy_v1.md`도 같이 갱신(반려 4사유 한국어 라벨 등) |
...
> **표 이해**: 화면·AC·플로우의 **원문은 PRD**가 기본이다. 정책·수치·라벨 캐노니컬은 **Policy**. **PC 와이어**(`ProudGallery_Screen_Spec_v1.html`)는 시각 파생물이며 IA·PRD와 동기화한다.
...
| 표준어 | 정의 | 옛 표현·금지 표기 |
|---|---|---|
| **회원** | Proud Gallery에 가입한 사용자 | — |
| **비회원** | 가입 전 사용자, 또는 그룹 전시의 비회원 참여자 | ~~비가입자~~ |
| **참여 작가** | 여러 작가 함께 올리기(그룹 전시)에서 같이 올라간 작가 | ~~참가자~~ |
| **작가 미상** | 슬롯 수동 해제(§3.5) 시 표시되는 라벨. 탈퇴 시에는 슬롯 자체가 삭제됨(§4.2). | — |
...
| **여러 작가 함께 올리기** | 여러 작가가 같이 올리는 그룹 전시. 전시에 등장하는 서로 다른 작가 2명 이상 필수. 게시자 본인 작품 포함 시 타인 1명 이상, 미포함 시 타인 2명 이상. | ~~함께 올리기~~ ~~그룹 업로드~~ |

### 5) 큐레이션

| 표준어 | 정의 | 비고 |
|---|---|---|
| **Pick** (Proud's Pick) | 운영팀이 매주 선정하는 우수 작품 | 주간 10건 상한, 영구 이력 배지 |
...
| **온보딩** | 가입 직후 다단 흐름 — Step 0 환영 · Step 1 프로필 입력 · (조건부) Step 2 본인 작품 찾기 · Step 3 완료 축하 (USR-AUT-09 → USR-AUT-10 → USR-AUT-10b → USR-AUT-11) | — |
...
| 항목 | 내용 |
|---|---|
| 수익화 모델 | **Phase 1 무료.** 결정 없음 |
| 계정 모델 | 단일 사용자 계정(가족 계정 없음). 탈퇴 시 본인 이미지 슬롯 삭제, 이미지 0장이 된 전시만 삭제 |
| 소셜 로그인 | 카카오 · 구글 · 애플 (Phase 1 모의 → 런칭 전 백엔드 연동 시 실연동) |
| 이메일 로그인 | 지원. Phase 1 모의 → 런칭 전 백엔드 연동 시 이메일 소유권 인증 |
| 최소 연령 | 만 14세 |
| 역할 | 비로그인 사용자 / 일반 회원(작가) / 운영자 |

### 주요 기능

- 전시 업로드: 내 작품 올리기 / 여러 작가 함께 올리기(참여 작가). 1~10장 이미지. 카메라 사진(EXIF 감지) 차단.
- 둘러보기 피드: 5 버킷 인터리빙(Pick·추천 전시·개인화·신규·일반). 기획전·응모전은 별도 표면(USR-CUR-01·USR-EVT-01) 노출.
- 큐레이션: Pick(주간 10 · 영구 이력 배지) · 기획전(묶음) · 응모전(응모 + 선정).
- 링크 공유 카드: 전시 링크를 카톡·문자·SNS에 올릴 때 서비스 공통 이미지로 노출.
...
- 신고·모더레이션 (Phase 1): **3액션(삭제·기각·비공개 유지)**, 운영팀 직접 판정. 사용자 계정 제재 확장은 [Policy §12.3](Policy_v1.md#12-3-phase-1-범위-밖-추후-이관) 범위 밖.
- 이미지 저작권 보호: 우클릭·드래그·길게누름 차단, 피드·카드는 1200px 썸네일 제공(원본 해상도 직접 노출 금지), 파일명에 작가명·전시명 등 식별 정보 미포함.
- 알림: 사용자가 가진 연락처 기준으로 3단계 자동 라우팅 — ① 한국 전화번호 보유자: 카카오 알림톡(SMS 폴백) ② 이메일만 보유: 이메일 ③ 둘 다 없음(카카오 + 전화번호 동의 거부 + 이메일 미등록): 인앱 알림만. 상세: [Policy §1.1](./Policy_v1.md#1-1-채널-라우팅).
...
- 법적 의무 알림(약관·개인정보 변경): §1.1 채널 라우팅 기준 발송 ([Policy §1.3](./Policy_v1.md#1-3-약관·처리방침-변경-고지-정책))
...
| 버전 | 일자 | 작성 | 변경 내용 |
|------|------|------|----------|
| v1.15 | 2026-05-19 | PM × Claude | 업로드 유형 표준어 갱신 — "혼자 올리기"→"내 작품 올리기", "함께 올리기"→"여러 작가 함께 올리기"; 폐기 별칭 strikethrough 추가 |
| v1.14 | 2026-05-09 | PM × Claude | 검수 상태 5종→4종, 함께 올리기 조건 명확화, 링크 공유 카드 수정; 충돌 방지 구조표 알림 10→13종(신고 처리 작가 수신 3종 추가, 신고자 기각 알림 제거); 충돌 방지 구조표 "그룹 초대"→"그룹 전시 게시", "초대"→"작품 연결" 정정; 핵심 결정 알림 라우팅 3단계로 갱신; 언어 정책 법적 의무 알림 채널 §1.3 정합; 「코드 주도 변경 시 HTML 동기화 의무」규칙 추가 — MD 변경 없이 코드에서만 UI 요소 제거·변경 시에도 HTML 슬라이드 같은 커밋에서 수정 의무 명문화 |
| v1.13 | 2026-05-07 | PM × Claude | 용어 사전 온보딩 정의 — Step 0.5·USR-AUT-09b 제거, 3단계 흐름으로 정정; 문서 제목·서비스명·본문 Artier→Proud Gallery 정합 |
| v1.12 | 2026-05-05 | PM × Claude | 용어 사전 기획전·응모전 표준어, 피드 5버킷 정합, 문서 이력 간소화 규칙, 인용 인라인 링크 통일, Handoff_Footer·Handoff_ServiceIntro → Copy §10·§11 흡수, 참조 링크 정정 |
...
| v1.5 | 2026-05-01 | PM × Claude | **화면 스펙 HTML 등록** — `_screen_specs/ProudGallery_Screen_Spec_v1.html`을 「기획 문서 목록」에 추가. **법무 검토용 체크리스트 등록** — `Handoff_LegalReview_Checklist_v1.md`를 「기획 문서 목록」에 추가. **핸드오프 신규 2종** — `Handoff_Onboarding_v1.md`(가입 후 4단계 + claim 흐름) · `Handoff_Notifications_v1.md`(인박스 + 검수·신고·Pick·초대 알림 + 채널 라우팅 매트릭스). |
