...
전시 1건이 삭제되면(사용자 자가 삭제 · 어드민 삭제 · 신고 처리 "삭제" 모두) 해당 ID를 참조하는 **모든 저장소의 레코드를 즉시 정리**해 stale 참조가 남지 않도록 한다. Phase 1은 클라이언트 단일 소스라 즉시 원자적 정리 가능. Phase 2 백엔드 이관 시 트랜잭션·cascade FK 또는 배치 정리로 대체한다.
...
- Phase 2 백엔드 이관 후에도 블루/그린 또는 롤링 배포로 무중단 유지.
