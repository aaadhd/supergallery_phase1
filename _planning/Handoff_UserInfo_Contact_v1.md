...
> **단일 소스 안내**
> - 화면에 찍히는 문자열(i18n)은 [Copy_v1.md] 문의 영역이 단일 소스다.
> - 본 문서의 한글·영문 원고는 Copy와 **같은 의미**를 유지해야 하며, 문자열 수정 시 Copy를 먼저 바꾼 뒤 본 문서를 같은 작업 범위에서 맞춘다.
> - 카테고리 **8종**은 [PRD_User USR-INF-07](PRD_User_v1.md)·[Policy §30]·[Policy §33] 정합. 특히 "개인정보 열람·정정·삭제 요청"은 법령 권리 행사 채널이므로 운영팀 SLA가 별도 적용. "닉네임 변경 신청"은 [Policy §4.4]·USR-INF-07 AC-06·07과 연동.
...
| 항목 | 결정 | 근거 |
|---|---|---|
| 일반 문의 SLA | **영업일 5일** 이내 응답 | [Policy §30.0] |
| 개인정보 권리 행사 SLA | 접수 5영업일 + 처리 30일 (1회 연장) | [Policy §30.3] |
| 첨부 파일 | 최대 3개, 각 5MB 이하 | 운영 보안 정책 |
| 본인 확인 | 로그인 상태는 세션 1차 확인. 비로그인·탈퇴자는 가입 이메일에서 회신 | [Policy §30.4] |
...
아래 문구는 [Copy_v1.md] 문의 영역과 동일 의미로 맞춘다.
...
| 4 | 신고/저작권 관련 | [Policy §22.2] 영업일 24시간 (위급) |
| 5 | **개인정보 열람·정정·삭제 요청** | [Policy §30.3] 접수 영업일 5일 + 처리 30일 |
...
| 4 | Report / copyright | [Policy §22.2] 24 business hours (urgent) |
| 5 | **Personal data access / correction / deletion** | [Policy §30.3] 5 business days + 30 days resolve |
...
**실제 i18n 원본**은 [Copy_v1.md] 문의 영역 한 곳이다. 코드에는 그걸 쓰면 된다. 본 문서 §2·§3은 기획이 의미를 고정해 두는 참고용 원고이며, 카테고리 순서·라벨 의미는 §2.3·§3.3과 Copy가 어긋나면 안 된다. 문구 변경 시 [README](README.md)의 카피 번들 규약대로 Copy를 먼저 수정한다.
...
| 화면 | 볼 곳 |
|---|---|
| 설정 USR-STG-01 | [PRD_User_v1.md] §15 |
| 프로필 편집 USR-PRF-02 | [PRD_User_v1.md] §6.2 |
...
- [ ] 카테고리별 어드민 라우팅 (개인정보 카테고리는 [ADM-INQ-01] 개인정보 우선 큐로 분류)
...
- 카테고리 2 "닉네임 변경 신청" — [Policy §4.4]·[PRD USR-INF-07](PRD_User_v1.md) AC-06·07(본인 확인·희망 닉네임·사유)과 정합.
- 카테고리 5 "개인정보 열람·정정·삭제 요청" — [Policy §30] 권리 행사 채널의 단일 진입점. 운영팀 본인 확인 절차([Policy §30.4])와 정합 필수. 변호사 검토는 [Handoff_LegalReview_Checklist §3 LP-7] 참조.
- 어드민 측 처리 화면 — [ADM-INQ-01] 문의함 + [Policy §22.7] 감사 로그 기록
- 신고·저작권 카테고리는 위급 상황 가능 → 처리 시한이 다른 카테고리보다 짧음 ([Policy §22.2])
