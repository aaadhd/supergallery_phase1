...
/demo/reference|모의 JWT·이메일 템플릿·발송 조건 표`,
  ...
  'settings.notifChannelPolicy':
    '밖으로 나가는 안내는 알림톡·문자·이메일 순으로 보내요. 카카오로 가입하셨다면 알림톡·문자를 먼저 쓰고, 그 밖의 가입은 주로 이메일이에요.',
...
/demo/reference|Mock JWT, email templates, send-rule table`,
  ...
  'settings.notifChannelPolicy':
    'Outbound notices try Alimtalk, then SMS, then email. Kakao sign-ups use Alimtalk/SMS first; other sign-ups mainly use email.',
