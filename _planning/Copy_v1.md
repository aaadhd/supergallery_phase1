# Copy Guide (UX Writing) · v1

> Artier(SuperGallery Phase 1)의 UX 문구 가이드. 본 문서는 **톤·구조 기준(§1~5, §7~8)**과 **전체 i18n 사전(§6)**을 단일 소스로 제공한다. 구현은 §6의 한국어·영어 오브젝트를 그대로 TypeScript로 옮기고, §7 기술 규칙을 따른다.
...
> 코드상 `Work` 타입 = UX상 "전시". 사용자 카피엔 **절대 "Work" 노출 금지**.
...
| 작품명·전시명·그룹명 | **20자** (`TITLE_FIELD_MAX_LEN`) | [Policy §9](Policy_v1.md) |
...
### 4.1 확인 다이얼로그 (CM-01 / `openConfirm`)
...
5. **아이콘 단독 의존 금지**: 모든 아이콘 버튼에 `aria-label` + 시각 레이블 병기 권장.
...
모든 키 = **한국어 · 영어 1:1 쌍**. 이 섹션이 카피 단일 소스이며, 구현 시 TypeScript 오브젝트 2개(`koMessages`, `enMessages`)로 매핑한다. 키 추가·수정 시 두 언어 동시 반영.
...
/demo/reference|모의 JWT·GeoIP·이메일 템플릿·발송 조건 표`,
  ...
  'upload.reviewInfoReject': '반려되면 사유를 함께 알려드려요. 수정 후 다시 발행하면 새로 검수해 드려요.',
  ...
  'upload.autoSavedAt': '자동 저장됨 · {time}',
  'upload.publishedConfirmTitle': '전시가 등록되었어요',
  'upload.publishedConfirmTitleApproved': '전시가 등록되었어요',
  'upload.publishedConfirmDesc': '운영팀 검수 후 둘러보기에 노출됩니다. 검수는 최대 24시간이 소요됩니다.',
  'upload.publishedConfirmDescApproved': '작품이 둘러보기 피드에 바로 노출됩니다.',
  ...
  'passwordReset.sent':
    '등록된 연락처로 비밀번호 재설정 링크를 발송했습니다. 이메일이 등록되어 있다면 곧 도착합니다. (유효시간: 1시간)',
  ...
  'settings.changePasswordDialogTitle': '비밀번호 재설정',
  'settings.changePasswordDialogBody': '{email}로 재설정 링크를 보내드릴게요. 메일함에서 확인 후 새 비밀번호를 설정할 수 있어요.',
  'settings.changePasswordSend': '재설정 링크 보내기',
  'settings.changePasswordSocialHint': '소셜 계정은 각 서비스에서 비밀번호를 관리해주세요.',
  'settings.changePasswordToast': '{email}로 재설정 링크를 보냈어요. 메일함을 확인해주세요.',
...
/demo/reference|Mock JWT, GeoIP, email templates, send-rule table`,
  ...
  'upload.autoSavedAt': 'Auto-saved · {time}',
  ...
  'upload.publishedConfirmDesc': 'It will appear on Browse after review. This usually takes up to 24 hours.',
  'upload.publishedConfirmDescApproved': 'Your work is now visible on the Browse feed.',
  ...
  'settings.changePasswordDialogTitle': 'Reset password',
  'settings.changePasswordDialogBody': 'We\'ll send a reset link to {email}. Check your inbox to set a new password.',
  'settings.changePasswordSend': 'Send reset link',
  'settings.changePasswordSocialHint': 'Social accounts manage passwords on their own provider.',
  'settings.changePasswordToast': 'Reset link sent to {email}. Check your inbox.',
...
- **점 구분 네임스페이스**: `{scope}.{kind}[.{variant}]`. 예: `upload.errFileTooBig`, `profile.emptyWorksManageHint`.
- **scope**: `upload`, `profile`, `work`, `exhibition`, `auth`, `onboarding`, `search`, `invite`, `admin`, `confirm`, `error`, `nav`, `social`, `brand`.
- **kind**: `title`, `description`, `action`, `error`, `empty`, `help`, `subtitle`, `label`, `placeholder`, `done`, `toast`, `badge`.
- **에러 키**: `err` 접두 (`upload.errMaxImages`). 에러가 아닌 경우엔 `err` 금지.
...
- **반드시** `useI18n()`의 `locale`·`t`를 렌더에서 사용. 스냅샷 함수(`getStoredLocale()`) **직접 호출 금지** — 런타임 언어 전환 시 리렌더 안 됨 (CLAUDE.md "코딩 규칙" 참조).
- 신규 키 추가 시 ko·en 동시 업데이트. 한쪽만 추가하면 타입은 통과해도 누락 locale 쪽 런타임 fallback은 키 문자열 자체가 노출됨.

### 7.4 복수·존댓말

- **복수형 없음** — 한국어 관용상 단/복수 구분 약함. 영어는 단수/복수 분기 필요 시 키 2개 분리(`work.oneLike` / `work.nLikes`) — 현재 Phase 1은 대부분 카운트 숫자 + 단위로 처리.
