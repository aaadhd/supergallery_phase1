...
> {provider} 계정이 확인되었어요. 약관에 동의하면 가입이 완료돼요.
...
- 약관·개인정보 텍스트 옆 "보기" 버튼 → 각각 경로 /terms, 경로 /privacy 이동
- 가입 완료 후 온보딩(USR-AUT-10 프로필 입력)으로 직접 이동. 소셜 제공자 표시명은 artier_pending_signup_nickname에 보관되어 온보딩 닉네임 필드에 prefill됨. 별도 안내 카피 없음.

---

## 3) 만 14세 미만 차단 안내 (로그인·가입 화면 공통)

**위치**: USR-AUT-04 약관 폼 / USR-AUT-06 모달 내 약관 체크박스
...
→ "만 14세 이상이에요" 체크박스가 법적 자기 선언 역할. 생년월일 수집 없음.
...
- [ ] "만 14세 이상이에요" 체크박스 미동의 시 가입 버튼 비활성 (생년월일 수집 없음)
