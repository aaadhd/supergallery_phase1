...
### 1.3 카테고리 필터 (유형 전체 + 유형 6개 — 표 7행. PRD USR-NTF-01 SSoT 정합)
...
| 시스템 | 시스템 | System | 검수 통과(업로더·참여자) + 검수 반려 + 작품 연결 + 신고 처리 결과(작가 수신) |
...
전시 신청 → 검수 → 승인/반려 흐름에서 작가에게 도달하는 알림.

### 2.1 검수 시작 (작가에게 신청 직후)
...
→ 신청 직후 1건. 검수 SLA([Policy §22.1](./Policy_v1.md#22-1-검수-sla)) 영업일 24시간 인지.
...
→ 운영팀이 매주 Pick 선정 시 작가에게 1건. 강제 수신 (토글 없음, 항상 발송). 카테고리: **큐레이션**. 단위: 전시.
...
→ 운영팀이 기획전 편집에서 piece를 추가할 때 그 piece의 작가에게 1건. 강제 수신 (토글 없음, 항상 발송). 카테고리: **큐레이션**. 단위: 작품(piece). **영구 배지는 부여하지 않음** (Policy §15.2 — 기획전 = 배지 없음). 알림으로만 인지.
...
|---|---|---|---|
| 내 작품 반응 알림 (좋아요·팔로우) | 가능 | ON | `settings.notifReactionAlerts` |
| 이벤트 알림 (응모전 공지·이벤트) | 가능 | OFF | `settings.notifMarketing` |
| 기획전·Pick | 불가 (강제 수신) | 항상 | — |
...
→ 비회원 작가가 가입 직후 "본인 작품 찾기"에서 자리 클레임 시 작가에게 1건. 정보용. 별도 액션 진입점 없음(잘못 연결됐을 때 작가가 마이페이지 슬롯 편집으로 처리, [Policy §3.5](./Policy_v1.md#3-5-잘못-연결됐을-때)).
...
|---|---|---|---|---|---|
| 검수 접수(신청 직후) | ✓ | — | 자동 라우팅 | 자동 라우팅 | 신청 직후 1건 |
| 검수 승인(작가) | ✓ | 자동 라우팅 | 자동 라우팅 | 자동 라우팅 | 검수 승인 시 |
| 검수 승인(클레임 비회원 작가) | ✓ | 자동 라우팅 | 자동 라우팅 | 자동 라우팅 | 클레임된 비회원 작가에게 |
...
| Pick·기획전 선정 | ✓ | 자동 라우팅 | 자동 라우팅 | 자동 라우팅 | 강제 수신 (토글 없음, 항상 발송) |
| 초대 클레임(작가) | ✓ | 자동 라우팅 | 자동 라우팅 | 자동 라우팅 | 비회원 작가가 클레임했을 때 |
...
| 버전 | 일자 | 작성 | 변경 내용 |
|------|------|------|----------|
| v10 | 2026-05-18 | PM × Claude | §2.5 "새 전시 알림(팔로워)" 삭제 — PRD 13종 미포함 확인; §2.1·§7 "발행" → "신청" 용어 정비 |
| v9 | 2026-05-14 | PM × Claude | §7 채널 매트릭스 Pick·기획전 선정 "기본 ON·OFF 가능" → "강제 수신 (토글 없음)" 정정 |
| v8 | 2026-05-14 | PM × Claude | "친구" → "비회원 작가" 용어 전체 변경(섹션명·표·체크리스트) |
| v7 | 2026-05-09 | PM × Claude | Pick·기획전 토글 기본 ON 정비, §4.5.3 요약표 신설; §10 필터 칩 명칭 정합(Pick→큐레이션, 초대→그룹 초대); §3 신고자 기각 알림 제거 — 작가 수신 3종만 유지, 섹션 번호 정합; §4.5.3 토글 표·§10 체크리스트 잔존 "그룹 초대"→"그룹 전시 게시" 정정; A-3 알림 설정 개편 — §4.5.3 토글 표 2종+강제 수신 구조로 교체(notifReactionAlerts·notifMarketing 선택, 그룹전시게시·기획전·Pick 강제 수신); §4.6 그룹 전시 게시 토글→강제 수신으로 정정 |
