...
- `src/app/utils/inviteMessaging.ts` — 초대 발송 (5% 랜덤 실패 시뮬, `artier_invite_messaging_log`) + `findMatchCandidates(phone)`로 전화번호 일치 초대 슬롯을 후보로 모음 → Onboarding step 2 본인 확인 단계(Policy §3.5.1) 거쳐 `applyConfirmedMatches`로 회원 슬롯 승격(실명 대조 폐기). `demoteSlotToUnknown`으로 마이페이지 disavow 시 슬롯을 `'unknown'` (작가 미상)으로 원복 + 발신자에게 알림(경고 없음).
...
## 초대 자동 연결 정책 (Policy §3.5 · v2.13 본인 확인 단계 재도입)

- **본인 확인 후 연결**: Onboarding step 2에서 가입자가 입력한 전화번호와 일치하는 초대 후보(`imageArtists[i] type === 'non-member'` + 전화 일치)가 발견되면 **본인 확인 화면**(Policy §3.5.1)을 같은 step 자리에 노출. "맞아요" 선택 시 `applyConfirmedMatches`로 모든 후보 슬롯을 회원으로 승격 + 발신자에게 시스템 알림. "아니에요" 선택 시 `recordDeclinedMatches`로 매칭 미적용 + 발신자 신호. 만 14세 검증·실명 대조는 폐기(Policy §2.2 v2.13: 자가 입력 + 4단 정책 가드).
...
- **타입**: `ImageArtistAssignment.type` = `'member' | 'non-member' | 'unknown'` (`src/app/data.ts`). `'unknown'` 슬롯은 부가 필드 없으며 렌더는 `t('work.unknownArtist')` ("작가 미상")로 통일.
- **Profile 노출 규칙**: 본인 확인을 통해 연결된 storeWorks도 `artistWorks` 버킷에 포함 (`Profile.tsx` `storeParticipating`, 2026-04-19). 이전엔 `hydrateGroupWorks` 시드만 참여 작품으로 인식해 연결 결과가 마이페이지에 노출되지 않았음.
- **Upload 편집 시 'unknown' 슬롯**: 로드·저장에서 원상 보존(`src/app/pages/Upload.tsx`, `artistType: 'unknown'`). 그룹 업로드 필수 작가 검증에서 'unknown'은 유효 상태로 통과. 편집 UI에 "작가 다시 지정하기" 버튼 제공.
- **온보딩 완료 화면 분기**: 본인 확인 후 연결 성공한 경우 step 3에 분기 카피(`onboarding.doneInviteMatchedHeadline/Body`) 노출 — 마이페이지 진입 동선 안내.
