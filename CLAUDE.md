...
PM 결정이 영향을 받는 작업(카피 작성·정책 정정·기획 변경·핸드오프 문서 수정 등)을 시작하기 전 **반드시 `_planning/README.md`를 먼저 읽고** 다음 두 영역을 확인한다.

1. **「용어 사전」** — 표준어 사용 여부 점검. 옛 표현 재사용 금지(예: ~~참가자~~ → 참여 작가, ~~비가입자~~ → 비회원, ~~운영 비공개~~ → 비공개 유지, ~~그룹 업로드~~ → 함께 올리기). 본 사전에 없는 새 용어가 필요하면 사전에 등록 후 사용.
2. **「기획 문서 목록」 + 「충돌 방지 구조」** — 어느 문서를 단일 소스로 다룰지 확인. 카피 변경은 `Copy_v1.md`(i18n 키 SSoT) ↔ Handoff prose 핸드오프본 양측 동시 갱신.
