...
> 이 문서는 실제 코드(`src/app/`)를 기반으로 역추적한 서비스 명세이다. 추측 없이 코드에 존재하는 것만 기술한다.
> 생성일: 2026-04-18
...
- 비회원 작가에게 SMS/카카오 초대 → 가입 시 자동 매칭
- 운영팀 어드민 콘솔(검수, 큐레이션, 신고 처리, 배너 관리 등)

Phase 1은 **클라이언트 전용**(백엔드 없음)이며, 모든 데이터는 localStorage/sessionStorage에 저장된다. OAuth, SMS, 이메일 발송은 모의(mock) 수준이다.
...
- 커스텀 커버 이미지 별도 지정
- 이미지별 작가 지정: 회원(검색) 또는 비회원(이름+전화번호 → SMS 초대)
- 전시명, 그룹명, 작품명 입력 (각 20자 이내)
- "저는 강사예요" 체크박스 (함께 올리기 → 세부정보 모달)
- 이벤트 연결 (linkedEventId)
- 초안 저장 / 초안에서 복원
- 기존 작품 수정
...
- 최대 10장
- 카메라 EXIF 감지 시 업로드 차단 (cameraExifBlock.ts — JPEG만 검사, Make/Model 태그)
- 비속어 필터 적용 (profanityFilter.ts — 47개 금칙어)
- **강사 OFF**: 그룹 전시에 본인 작품 최소 1점 필수
- **강사 ON**: 본인 작품 포함 금지 (자동 필터링 + 슬롯 초기화)
- 미저장 작품 이탈 방지 (beforeunload + router block)
- 업로드 시 기본 상태: `feedReviewStatus: 'pending'` (환경변수 `VITE_UPLOAD_AUTO_APPROVE=true` 시 즉시 `approved`)
- 발행 시 AP +20 (첫 업로드는 +100)
- 24시간 이내 삭제 시 AP -20 회수

### 3.3 Profile (프로필) — `/profile`, `/profile/:id`, `/me`

**할 수 있는 것:**
- 탭 전환: 전시 / 작품 / 수강생 작품(강사만) / 좋아요 / 저장 / 초안
- 전시 필터: 전체 / 혼자 / 함께
- "내 작품만 보기" 체크박스
- 프로필 편집: 소개글, 바이오, 지역, 관심사, 외부 링크
- 개별 작품명 수정 (혼자 올린 작품의 이미지별)
- 팔로워/팔로잉 모달
- 작품 삭제
- 팔로우/언팔로우

**규칙:**
- 강사 배지: `isInstructorUpload === true`인 작품이 하나라도 있으면 자동 노출
- "수강생 작품" 탭: 강사가 올린 그룹 작품만 표시
- `pending`/`rejected` 상태 작품은 소유자 외에게 비노출
- 탈퇴 작가는 "삭제된 사용자" 표시
...
- 소셜 최초 가입 → SocialSignupModal (약관 동의 + 닉네임) → 온보딩
...
- 비밀번호 변경 (이메일 로그인 사용자만; 소셜 사용자는 안내 표시)
- 로그아웃
- 회원 탈퇴 (비밀번호 + 탈퇴 사유 필수)
...
### 3.15 Points (포인트) — `/points`

**할 수 있는 것:**
- AP (활동 포인트) / PP (프리미엄 포인트) 잔액 확인
- 내역 필터: 전체/적립/사용
- 거래 내역 (금액, 설명, 시각)
...
### 4.3 작품 업로드 플로우
```
Upload 진입 → 유형 선택 (혼자/함께)
  → 이미지 추가 (1~10장, 800px+, EXIF 차단)
  → 이미지별 작가 지정 (회원 검색 또는 비회원 초대)
  → 세부정보: 전시명, 그룹명, 작품명, 강사 체크, 이벤트 연결
  → 발행
    → feedReviewStatus: 'pending' (검수 대기)
    → 검수 승인 후 피드 노출
    → AP +20 (첫 업로드 +100)
  → 비회원 작가에게 SMS/카카오 초대 발송 (5% 랜덤 실패 시뮬)
```
...
운영팀: /admin/reports
  → 삭제: 작품 영구 삭제
  → 경고: 대상 작가 경고 +1 (3회 → 자동 7일 정지)
  → 기각: 신고자 허위 신고 +1 (3회 → 신고자 7일 차단)
  → 비공개: 작품 isHidden (피드 제외, 본인만 보임)
```
...
| 강사 OFF + 그룹 전시 | 본인 작품 최소 1점 포함 필수 | `Upload.tsx` |
| 강사 ON | 본인 작품 포함 금지 | `Upload.tsx` |
...
| 규칙 | 값 |
|------|-----|
| 채널 판정 | 한국 번호(+82, 82, 01x) → 카카오 알림톡; 그 외 → SMS |
| 중복 발송 | 동일 (전화번호 + workId) 차단 |
| 실패 시뮬 | 5% 랜덤 실패 (데모용) |
| 매칭 로그 | 최대 200건 |
| 초대 로그 | 최대 300건 |
