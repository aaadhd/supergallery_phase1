...
- "저는 강사예요" 체크박스 (본인 슬롯 초기화 전 확인 다이얼로그)
...
- **강사 ON**: 본인 작품 포함 금지 (확인 다이얼로그 후 슬롯 초기화)
...
**할 수 있는 것:**
- 탭 전환: 전시 / 작품 관리 / 수강생 작품(강사만) / 좋아요 / 저장 / 초안
- 전시 필터: 전체 / 혼자 / 함께
- "내 작품만 보기" 체크박스
- 프로필 편집: 소개글, 바이오(200자), 지역, 관심사, 외부 링크 (연필 아이콘 버튼)
- 프로필 사진 변경 (눈에 띄는 카메라 아이콘)
- 개별 작품명 수정 (빈 문자열/공백 불가)
- 팔로워/팔로잉 모달 (전체 목록)
- 작품 삭제 ("영구 삭제" 경고 + 성공 토스트, 보류 초대 시 추가 경고)
- 초안 삭제 ("영구 삭제" 경고 + 성공 토스트)
- 팔로우/언팔로우
...
- `isHidden` 작품은 타인 프로필에서 비노출 (본인만 볼 수 있음)
- 탈퇴 작가는 "작가 미상" 표시, 인터랙션(좋아요/저장/팔로우/신고) 차단
- 자기 자신 팔로우 불가, 탈퇴 작가 팔로우 불가
...
Upload 진입 (기존 초안 감지 시 "이어서 작업" 토스트)
  ...
  → 이미지별 작가 지정 (회원 검색 또는 비회원 이름+전화번호)
  → 세부정보: 전시명(필수), 그룹명, 작품명, 강사 체크, 이벤트 연결
  → 전시하기
