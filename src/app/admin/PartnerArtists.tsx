...
import { PARTNER_STAGE, PARTNER_SUBMISSION } from './constants';
...
export default function PartnerArtists() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">파트너 작가 관리</h1>
        <p className="text-sm text-muted-foreground mt-1">모집/온보딩/관리 및 월간 루틴 추적</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg border p-4">
          ...
          <p className="text-xs text-muted-foreground mt-1">목표: {targetArtists}명</p>
        </div>
        <div className="bg-white rounded-lg border p-4">
          <p className="text-xs text-muted-foreground">활성 작가</p>
          <p className="text-2xl font-bold text-primary">{activeCount}명</p>
        </div>
        <div className="bg-white rounded-lg border p-4">
          <p className="text-xs text-muted-foreground">제출 작품</p>
          <p className="text-2xl font-bold">{totalWorks}점</p>
        </div>
        <div className="bg-white rounded-lg border p-4">
          <p className="text-xs text-muted-foreground">온보딩 중</p>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(partner => (
                <TableRow key={partner.id}>
                  ...
                  <TableCell className="text-xs text-muted-foreground">{partner.contactStatus}</TableCell>
                  <TableCell>
                  ...
                  <TableCell className="max-w-[180px]">
                    {editingNote === partner.id ? (
                      ...
                      <span
                        className="text-xs text-muted-foreground truncate block cursor-pointer lg:hover:text-foreground"
                  ...
                  <TableCell className="text-xs text-muted-foreground">{partner.lastContactAt}</TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
