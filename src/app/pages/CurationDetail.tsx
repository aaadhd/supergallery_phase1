              ...
              <>
                ...
                <div className="absolute bottom-0 left-0 right-0 px-6 py-5 pointer-events-none">
                  <div>
                    ...
                    {curation.subtitle && (
                      <p className="text-white/40 text-[13px] leading-snug mt-0.5">{curation.subtitle}</p>
                    )}
                    {(bannerArtistNames.length > 0 || (curation.startAt && curation.endAt)) && (
                      <div className="mt-4 flex flex-col space-y-1">
                        {bannerArtistNames.length > 0 && (
                          <p className="text-white/65 text-[13px] tracking-wide">{bannerArtistNames.join(' · ')}</p>
                        )}
                        {(curation.startAt && curation.endAt) && (
                          <p className="text-white/70 text-[13px] tracking-[2px]">{curation.startAt.replace(/-/g, '.')} — {curation.endAt.replace(/-/g, '.')}</p>
                        )}
                      </div>
                    )}
                  </div>
