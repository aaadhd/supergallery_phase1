    ...
    <div className="min-h-screen bg-white flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-md">
        ...
        <div className="space-y-3">
          {/* GeoIP 대체: locale 기반 소셜 로그인 버튼 순서 분기 (KR: 카카오 우선, 해외: 구글/애플 우선) */}
          {(locale === 'ko' ? ['kakao', 'google', 'apple'] : ['google', 'apple', 'kakao']).map((provider) => {
            if (provider === 'kakao') return (
              <Button key="kakao" type="button" variant="secondary" className={cn('h-11 min-h-11 w-full gap-3 text-base font-medium', 'border-0 bg-[#FEE500] text-[#3C1E1E] lg:hover:bg-[#FDD800]')} onClick={handleSocialLogin}>
            ...
            if (provider === 'google') return (
              <Button key="google" type="button" variant="outline" className="h-11 min-h-11 w-full gap-3 text-base font-medium" onClick={handleSocialLogin}>
            ...
            return (
              <Button key="apple" type="button" className="h-11 min-h-11 w-full gap-3 bg-foreground text-base font-medium text-white lg:hover:bg-foreground/90" onClick={handleSocialLogin}>
