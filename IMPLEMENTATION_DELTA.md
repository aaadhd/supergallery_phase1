...
| 기능 (Features) | 핵심 기능 구현 / 이벤트 단일 store·피드 큐레이션·팔로우 가중치 반영 | Pin 코멘트, 타임랩스, 색상 팔레트 모듈(Upload 미통합), 초대 발송 로그 등 | **명세 충족 + Phase 2 선행·데모** |
...
`public/images` 또는 manifest 변경 시 localStorage를 **자동 재초기화**하는 버전 관리 로직 (`WORKS_STORAGE_VERSION` 현재 값 `local-gallery-v10`, 키 `artier_works_version`). 명세 미포함이나 데모 환경 안정성 확보용.
...
- **11.1·11.2·11.3·11.4·11.5** — 완료
- **11.6** 배너 순서 UX (`order` + DnD) — 미구현
...
- **UI/UX 일관성** (ConfirmDialog, WorkCard, 초대 랜딩, 이벤트·큐레이션 스토어 등 — §10)
